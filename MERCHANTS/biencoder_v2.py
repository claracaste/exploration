...
#         text1 = self.dataframe.loc[index,'sentence']
#         text2 = self.dataframe.loc[index,'merchant_name_combined']
#         label = self.dataframe.loc[index,'true_label']

#         return {'sentence1': text1, 'sentence2': text2, 'score':label}

#     def __len__(self):
#         return len(self.dataframe)

# ds_train = PairsDataset(df_train)
# ds_val = PairsDataset(df_val)
# ds_test = PairsDataset(df_test)

# %%
from datasets import Dataset

ds_train = Dataset.from_pandas(df_train[['true_label','sentence','merchant_name_combined']])
ds_val = Dataset.from_pandas(df_val[['true_label','sentence','merchant_name_combined']])
ds_test = Dataset.from_pandas(df_test[['true_label','sentence','merchant_name_combined']])

ds_train = ds_train.rename_columns({"true_label": "score","sentence":"sentence1", "merchant_name_combined":"sentence2"})

ds_val = ds_val.rename_columns({"true_label": "score","sentence":"sentence1", "merchant_name_combined":"sentence2"})
...
dev_evaluator = EmbeddingSimilarityEvaluator(
    sentences1=df_val["sentence"],
    sentences2=df_val["true_merchant_name_combined"],
...
model.save_pretrained("models/model2-final")
...
# Example usage
local_directory = "models/model2"
bucket_name = 'cleo-data-science'
s3_directory = "transaction_enrichment/experimental_data/caste/pairs_model/model2"
